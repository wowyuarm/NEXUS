...
def verify_signature(payload: str, auth_data: dict[str, Any] | None) -> dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            recovered_public_key = signature.recover_public_key_from_msg_hash(
                message_hash
            )
        ...
        if recovered_address.lower() != public_key_hex.lower():
            logger.warning(
                f"Public key mismatch: expected {public_key_hex}, got {recovered_address}"
            )
            return {
                "status": "error",
        ...
        return {"status": "success", "public_key": public_key_hex}
