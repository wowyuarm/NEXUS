...
_identity_service_instance: Optional[Any] = None
_persistence_service_instance: Optional[Any] = None
_config_service_instance: Optional[Any] = None
...
def get_config_service():
    ...
    if _config_service_instance is None:
        ...
        raise HTTPException(
            status_code=503,
            detail="Service unavailable: ConfigService not initialized"
        )
...
async def verify_bearer_token(authorization: str = Header(None)) -> str:
    ...
    This function implements the RFC 6750 Bearer Token standard for authentication.
    The owner_key (public key) is used as the bearer token to identify the user.
    ...
    # Validate token format (Ethereum address starts with 0x)
    if not token.startswith('0x'):
        raise HTTPException(
            status_code=401,
            detail="Invalid token format (expected Ethereum address)"
        )
    
    logger.debug(f"Bearer token verified: {token}")
    return token
...
async def verify_request_signature(request_body: Dict[str, Any], owner_key: str) -> str:
    ...
    if not auth_data:
        raise HTTPException(
            status_code=401,
            detail="Missing authentication signature"
        )
...
async def get_prompts(
    ...
    try:
        ...
        return {
            ...
            "editable_fields": [f for f in profile["editable_fields"] if f.startswith("prompts.")],
            "field_options": {k: v for k, v in profile["field_options"].items() if k.startswith("prompts.")}
        }
    except Exception as e:
...
async def get_messages(
    limit: int = Query(20, ge=1, le=100, description="Number of messages to retrieve (1-100)"),
    owner_key: str = Depends(verify_bearer_token),
