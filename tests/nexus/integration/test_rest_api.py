...
class TestRESTAPIIntegration:
    ...
    def mock_identity_service(self, test_owner_key):
        ...
        async def mock_get_effective_profile(owner_key, config_svc):
            ...
            return {
                ...
                "user_overrides": {"config_overrides": {}, "prompt_overrides": {}},
                "editable_fields": [
                    "config.model",
                    "config.temperature",
                    "config.max_tokens",
        ...
        service.get_effective_profile = AsyncMock(
            side_effect=mock_get_effective_profile
        )
    ...
    def mock_config_service(self):
        ...
        service.get_user_defaults = Mock(
            return_value={"config": {"model": "gemini-2.5-flash"}, "prompts": {}}
        )
        service.get_genesis_template = Mock(
            return_value={
                "llm": {"catalog": {}},
    ...
    def mock_command_service(self):
        ...
        service.get_all_command_definitions = Mock(
            return_value=[
                {
                    "name": "config",
    ...
    def client(
        ...
        test_app.dependency_overrides[rest.get_command_service] = (
            lambda: mock_command_service
        )
        test_app.dependency_overrides[rest.get_identity_service] = (
            lambda: mock_identity_service
        )
        test_app.dependency_overrides[rest.get_config_service] = (
            lambda: mock_config_service
        )
        test_app.dependency_overrides[rest.get_persistence_service] = (
            lambda: mock_persistence_service
        )
    ...
    def test_get_config_with_invalid_bearer_token(self, client):
        ...
        response = client.get(
            "/api/v1/config", headers={"Authorization": "Bearer invalid"}
        )
        assert response.status_code == 401
    ...
    def test_get_config_success(self, client, test_owner_key, mock_identity_service):
        ...
        response = client.get(
            "/api/v1/config", headers={"Authorization": f"Bearer {test_owner_key}"}
        )
        ...
        # Verify prompt structure (v2 architecture - friends_profile only)
        assert "friends_profile" in data["effective_prompts"]
        assert "content" in data["effective_prompts"]["friends_profile"]
        assert "editable" in data["effective_prompts"]["friends_profile"]
        # friends_profile should be editable
        assert data["effective_prompts"]["friends_profile"]["editable"] == True
    ...
    def test_post_config_with_invalid_signature(
        self, mock_verify_signature, client, test_owner_key
    ):
        ...
        response = client.post(
            ...
            json={
                "overrides": {"temperature": 0.9},
                "auth": {"publicKey": test_owner_key, "signature": "0xinvalid"},
            },
        )
    ...
    def test_post_config_success(
        ...
        mock_identity_service.update_user_config.assert_called_once_with(
            test_owner_key, {"temperature": 0.9, "max_tokens": 8192}
        )
    ...
    def test_get_prompts_success(self, client, test_owner_key, mock_identity_service):
        ...
        response = client.get(
            "/api/v1/prompts", headers={"Authorization": f"Bearer {test_owner_key}"}
        )

        assert response.status_code == 200
        data = response.json()

        # Same structure as /config (v2 architecture - friends_profile only)
        assert "effective_prompts" in data
        assert "friends_profile" in data["effective_prompts"]
        # friends_profile should be editable
        assert data["effective_prompts"]["friends_profile"]["editable"] == True
    ...
    def test_post_prompts_success(
        self, mock_verify_signature, client, test_owner_key, mock_identity_service
    ):
        ...
        request_body = {
            "overrides": {"friends_profile": "User profile: I prefer concise answers..."},
            "auth": {"publicKey": test_owner_key, "signature": "0x" + "b" * 130},
        }
        ...
        mock_identity_service.update_user_prompts.assert_called_once_with(
            test_owner_key, {"friends_profile": "User profile: I prefer concise answers..."}
        )
    ...
    def test_get_messages_success(
        self, client, test_owner_key, mock_persistence_service
    ):
        """Test GET /messages returns message history."""
        response = client.get(
            "/api/v1/messages", headers={"Authorization": f"Bearer {test_owner_key}"}
        )
    ...
    def test_bearer_token_format_validation(self, client):
        ...
        response = client.get("/api/v1/config", headers={"Authorization": "0x1234"})
        assert response.status_code == 401

        # Empty token
        response = client.get("/api/v1/config", headers={"Authorization": "Bearer "})
        assert response.status_code == 401
    ...
    def test_signature_verification_called_correctly(
        ...
        request_body = {
            "overrides": {"temperature": 0.9},
            "auth": {"publicKey": test_owner_key, "signature": "0x" + "c" * 130},
        }
    ...
    def test_get_config_service_error(
        self, client, test_owner_key, mock_identity_service
    ):
        """Test GET /config handles service errors gracefully."""
        # Make service raise an exception
        mock_identity_service.get_effective_profile.side_effect = Exception(
            "Database error"
        )
    ...
    def test_post_config_update_failure(
        ...
        request_body = {
            "overrides": {"temperature": 0.9},
            "auth": {"publicKey": test_owner_key, "signature": "0x" + "e" * 130},
        }
    ...
    def test_post_config_invalid_request_body(self, client, test_owner_key):
        ...
        response = client.post(
            "/api/v1/config",
            headers={"Authorization": f"Bearer {test_owner_key}"},
            json={"auth": {"publicKey": test_owner_key, "signature": "0x..."}},
        )
