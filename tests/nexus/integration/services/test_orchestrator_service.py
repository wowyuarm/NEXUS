...
class TestOrchestratorFlows:
    ...
    async def test_simple_dialogue_flow(self, orchestrator_service, mock_bus, sample_run):
        ...
        assert orchestrator_service.active_runs[sample_run.id].status == RunStatus.BUILDING_CONTEXT
    ...
    async def test_single_tool_call_flow(self, orchestrator_service, mock_bus, sample_run):
        ...
        assert tool_message.content["args"]["query"] == "artificial intelligence definition"
        ...
        llm_message = llm_call[0][1]
        assert "messages" in llm_message.content
        assert len(llm_message.content["messages"]) == 3  # system, user, assistant, tool
        
        # Verify run state
        run = orchestrator_service.active_runs[sample_run.id]
        assert run.status == RunStatus.AWAITING_LLM_DECISION
        assert run.metadata['pending_tool_calls'] == 0
        assert len(run.history) == 3  # Original human + AI with tool calls + tool result

    @pytest.mark.asyncio
    async def test_multi_tool_sync_flow(self, orchestrator_service, mock_bus, sample_run):
        """
    ...
    @pytest.mark.asyncio
    async def test_max_iterations_safety_valve(self, orchestrator_service, mock_bus, sample_run):
        """
    ...
    @pytest.mark.asyncio
    async def test_handle_streaming_events_forwarding(self, orchestrator_service, mock_bus, sample_run):
        """
