...
class TestDeepSeekLLMProvider:
    ...
    def test_initialization_with_api_key(self, mocker):
        ...
        # Mock AsyncOpenAI
        mock_async_openai = mocker.patch('nexus.services.llm.providers.deepseek.AsyncOpenAI')
        mock_client = Mock()
    ...
    def test_initialization_with_no_api_key(self):
        ...
        with pytest.raises(ValueError, match="API key is required for DeepSeekLLMProvider"):
            DeepSeekLLMProvider(
                api_key="",
                base_url="https://api.deepseek.com"
            )
